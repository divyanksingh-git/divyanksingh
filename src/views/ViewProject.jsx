...
const ViewProject = ({ proj, theme }) => {
  const { iconSize } = usePortfolio(); // <-- Context!
    ...
    <div className="animate-in fade-in slide-in-from-right-4 duration-500 pb-10">
      ...
      <div className="grid lg:grid-cols-3 gap-6 md:gap-8">
        <div className="lg:col-span-2 space-y-6 md:space-y-8">
          <div className="bg-slate-900 dark:bg-slate-950 text-white p-6 md:p-10 rounded-[2rem] relative overflow-hidden shadow-2xl border border-white/5 group">
            <Terminal size={120} className="absolute -bottom-6 -right-6 opacity-[0.05] group-hover:opacity-10 transition-opacity duration-500 rotate-12 pointer-events-none" />
            <div className="relative z-10">
              <h3 className={`text-xs font-black uppercase tracking-widest ${theme.text} mb-4 flex items-center gap-2`}>
                 <FileCode size={14} className="shrink-0" /> Mission Brief
              </h3>
              <p className="text-lg md:text-2xl font-bold leading-relaxed text-slate-100">
                {proj.desc}
              </p>
            </div>
          </div>
